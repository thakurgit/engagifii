...
div#calendar_filter {
 ...
 <div id="calendar_div" class="position-relative">
    ...
    <script>
        ...
        function getEndorsementCalendar(target_div, year, month, day){
            $.ajax({
                type:'POST',
                url: engagifiiUrl_ajaxurl,
         
                data:{
              	action:'getendorsementcalendar',  
               	year: year,
               	month: month,
               	day:day,
                //courses : courses,
	...
	jQuery("#event_list .scroll> .btn").on('click', function(){
        ...
        $('.clear-all-cal').click(function(){
          ...
          $('.filter-icon-cal').removeClass('active');  
            ...
            courses = '';
            tags = '';
            getEndorsementCalendar('calendar_div', $('.year-dropdown').val(), $('.month-dropdown').val(), '');
    	...
    	function countFilterDataCal()
    	{
      		...
      		var tags = $.map($('input[name="endorsementTags[]"]:checked'), function(c){return c.value; });
          	$.ajax({
          		type : "post",
          		url: engagifiiUrl_ajaxurl,
          		data:{
