...
//print_r("event Grid");
        $postedData = $this->_classPostCountData();
        //print_r($postedData);
        $dataResponse = $this->submitApiRequest("public/list", $postedData, "POST", 'event');
...
//print_r($dataResponse);
        		...
        		foreach ($value->classSessions as $index => $class) {
		            ...
		            if($value->isClassRegistrationAllow)
		            {
		                if($value->registrationState !== 'Registration Not Setup' && $value->registrationState !== 'Registration Closed' && $value->registrationState!== 'Sold Out' && $value->registrationState !== 'Registration Scheduled' && $value->registrationState !== 'Early Sold Out' && $value->registrationState !== 'Standard Sold Out')
                        {
                            if($value->locationType->name=="onlocation")
                            {
                                $data['register'] = '<a href="'.$response->registrationUrlOnLocation.'" class="btn btn-primary px-3 py-1" target="_blank">Register</a>';
                            }
                            elseif($value->locationType->name=="online"){
                                $data['register'] = '<a href="'.$response->registrationUrlOnLine.'" class="btn btn-primary px-3 py-1" target="_blank">Register</a>';
                            }
                            elseif($value->locationType->name=="onlocationandonline")
                            {
                                $data['register'] ='<span id="classlocationButton" style="display: flex;"><a href="'.$response->registrationUrlOnLine.'" id="onlineclass" class="btn btn-primary px-3 py-1" target="_blank" style="margin-right:2px;">Register online</a><br/><a href="'.$response->registrationUrlOnLocation.'" id="onlocation" class="btn btn-primary px-3 py-1" target="_blank">Register in person</a></span>';
                            }
                            else{
                                $data['register'] = ' ';
                            }
                ...
                if($value->isClassRegistrationAllow)
	            {
	                ...
	                if($value->registrationState !== 'Registration Not Setup' && $value->registrationState !== 'Registration Closed' && $value->registrationState!== 'Sold Out' && $value->registrationState !== 'Registration Scheduled' && $value->registrationState !== 'Early Sold Out' && $value->registrationState !== 'Standard Sold Out')
                    {
                        if($value->locationType->name=="onlocation")
                            ...
                            elseif($value->locationType->name=="onlocationandonline")
                            {
                                $data['register'] ='<span id="classlocationButton" style="display: flex;"><a href="'.$response->registrationUrlOnLine.'" id="onlineclass" class="btn btn-primary px-3 py-1" target="_blank" style="margin-right:2px; ">Register online</a><a href="'.$response->registrationUrlOnLocation.'" id="onlocation" class="btn btn-primary px-3 py-1" target="_blank" >Register in person</a></span>';
                            }
                            else{
                                $data['register'] = ' ';
                            }
    ...
    public function eventsCalendar(){
        ...
        //$postedData = json_encode($postData);
           $dataResponse = $this->submitApiRequest("public/list", $postData, "POST", 'event');
...
//Display Calendar layout data for Endorsement : Added by Gurpreet 

public function getEndorsementCalendar(){
    //print_r("hello i am in endorsement calneder"); die;
    $year = $_POST['year'];
    $month = $_POST['month'];
    $day   = $_POST['day'] ? $_POST['day'] :date('d');
    $dateYear = ($year != '')?$year:date("Y");
