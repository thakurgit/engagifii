...
if(!empty($_POST['minRange']))
  ...
  public function eventsClassCalendar(){
	...
	if($classes_detail_page){
        ...
        $postData = array();    
        $postData['itemCount'] = $eventsClassCount;//$classCount;
        $postData['sortBy'] = 'sectionname';
        $postData['isUpcoming'] = $allEventsClass;
        $postData['pageNumber'] = 1;
        $postData['pageSize'] = ((int) $eventsClassCount);//((int) $classCount);
        $postData['sortDirection'] = 'asc';
        if(!empty($_POST['courses']))
        {
            $postData['courses'] = $_POST['courses'];
		...
		die;*/
           ...
           foreach ($collection as $key => $value) {
			   if($value->entity=='Event'){
				  ...
				  $registration_state = preg_replace('/(?<!\ )[A-Z]/', ' $0', $value->registrationState);//$value->eventRegistrationState;
				   foreach ($value->eventDates as $index => $event) {
					   //session start time and date
