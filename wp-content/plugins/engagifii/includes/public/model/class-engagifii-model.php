   ...
   public function generateDownloadsByPerson(){
	...
	public function downloadDataByPerson(){
        ...
        foreach ($collection as $key => $value) {
			...
			if($value->reportLink){
			$nestedData['download-select']='<div class="dropdown"><button type="button"  class="btn btn-sm shadow-none" data-toggle="dropdown"><img src="'.$download_icon.'" class="img-icon-lg rounded-circle img-fluid mr-2" alt="downlaod" style="width:25px;"/ ></button><div class="dropdown-menu py-1"><a class="dropdown-item px-2" target="_blank" href="'.$value->reportLink.'">Download</a><a class="dropdown-item px-2 deleteReport" href="" report-id="'.$value->id.'">Delete</a></div></div>';
...
foreach ($seqColumns as $key => $value) {
 ...
 /*  public function publicOfficialLoadData(){
		...
		//die;
         $dataResponse = $this->submitApiRequest("legislative/public-bills/elected/officials-all-tabs-list", $postedData, "POST", 'legislation');
        $collection   = json_decode($dataResponse['api_response'])->stateSenateMemberList;
		 //print_r($collection);
		 //die;
  ...
  /*  public function _preparePublicOfficialData(){
        $columnsData = [];
        foreach ($_POST['columns'] as $key => $value) {
            if ($value['orderable'] == "true") {
                $columnsData[$value['data']] = $value['data'];
