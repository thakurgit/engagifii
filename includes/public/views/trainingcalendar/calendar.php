...
$columnNames=[];
		if (!empty(EVENTS_CLASS_COLS) && isArrayOfJsonStrings(EVENTS_CLASS_COLS)) {
			  $columnNames = extractColNames(EVENTS_CLASS_COLS);
		}
?>
            ...
            if(response!= 'no-data') {
                ...
                obj.forEach(function(value, index) {
				   ...
				   if(value['entity']=='Class'){
                    class_html += '<div class="'+html_class+' mb-3  "><div class="box border rounded h-100 class-text bg-light"><div class="col-12 m-auto p-1 text-left d-flex align-items-center"><img src="'+value['icon']+'" class="img-fluid img-icon-lg mr-2" alt="image icon">'+value['title']+'<span class="badge badge-secondary ml-auto">'+value['entity']+'</span></div><div class="col-12 py-1 text-left">'+value["schedule"]+'</div><div class="col-12 py-1 text-left"><span class="text-muted">Type: </span><span>'+value["objectType"]+'</span></div> <div class="col-12 py-1 text-left"><span class=" text-muted">Duration: </span><span>'+value["classDuration"]+'</span></div><div class="col-12 py-1 text-left"><span class=" text-muted">Tags: </span><span>'+tags+'</span></div><div class="col-12 text-center py-3">'+value['viewdetails']+value['register']+'</div></div></div>';
				   }else{
                    class_html += '<div class="'+html_class+' mb-3  "><div class="box border rounded h-100 class-text bg-light"><div class="col-12 m-auto p-1 text-left d-flex align-items-center"><img src="'+value['icon']+'" class="img-fluid img-icon-lg mr-2" alt="image icon">'+value['title']+'<span class="badge badge-secondary ml-auto">'+value['entity']+'</span></div><?php if(in_array('startDateTime', $columnNames)) { ?><div class="col-12 py-1 text-left">'+value["schedule"]+'</div><?php } if(in_array('eventType', $columnNames)){ ?><div class="col-12 py-1 text-left"><span class="text-muted">Type: </span><span>'+value["objectType"]+'</span></div><?php }  if(in_array('tags', $columnNames)){ ?><div class="col-12 py-1 text-left"><span class=" text-muted">Tags: </span><span>'+tags+'</span></div><?php } ?><div class="col-12 text-center py-3">'+value['viewdetails']+' <?php if(in_array('register', $columnNames)){ ?> '+value['register']+'</div><?php } ?></div></div>';
				   }
