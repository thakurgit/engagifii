...
$options = get_option('ebt_api_settings');
	...
	if (!empty(EVENTS_CLASS_COLS) && isArrayOfJsonStrings(EVENTS_CLASS_COLS)) {
		  $columns = convertToObjectArray(EVENTS_CLASS_COLS);
...
if(isset($attr['calendar'])){
  $calendar_view = $attr['calendar'];
    ...
    $classdateRange  = $obj->eventClassdateFilters($date);
...
<div class="filter-content" id="filterdp1">
    ...
    <div class="col-sm-12" id="test">
      ...
      <?php if(in_array('startDateTime', $columnNames)) { ?>
       <div class="filter-list border-bottom">
        <div class="heading-title py-2 px-2 d-flex align-items-center justify-content-between"> Event Date <i class="far fa-angle-down"></i></div>
  ...
  <div class="filter-list border-bottom">
    <div class="heading-title py-2 d-flex align-items-center justify-content-between">
      Category <i class="far fa-angle-down"></i>
    </div>
    <div class="content-area d-none position-relative pb-2">
      ...
      <div class="mt-2">
        <label class="d-flex align-items-center">
          <input type="checkbox" name="category" value="event" class="mr-2"> <small>Event</small>
        </label>
        <label class="d-flex align-items-center">
          <input type="checkbox" name="category" value="class" class="mr-2"> <small>Classes</small>
        </label>
      </div>
       ...
       <?php }
      if(in_array('Type', $columnNames) && array_search('Type', $columnNames)){
      ?>
       <div class="filter-list border-bottom">
         ...
         // print_r($value);
          //if(in_array($value->colName, $ebt_visib_datacol_list)){
             ...
             /* $forDatatable[$i]['name'] =$value->colName;
              ...
              // if($value->colName == 'eventSchedule')
              // {
              //    $value->displayName = "Schedule";
              // }
...
var table = $('#ebtmaintable').DataTable( {
	   ...
	   //"search": {regex: true},
		...
		"order": [[<?php echo array_search('startDateTime',$columnNames);?>, 'desc']],
		 <?php } ?>
       "columnDefs": [ 
          { "targets": ['tags','register','type','city','status', 'entity', 'startdatetime'],
            "orderable": false
          },
		  { className: "title-col", "targets": "name" },
		  { className: "text-center", "targets": ["tags","register","Type","eventDates","city", "entity", "startdatetime", "status"] },
		  <?php if(in_array('startDateTime', $columnNames)){ ?>
        ...
        "ajax": {
          "url": ajax_url_evt,
            "type": "POST",
            "data": function(d) {           
            d.action = 'trainingCalendarGridData';   
              d.tags    = tags; 
              d.types    = types; 
              d.locations    = city; 
  ...
  $('.filter-list input[type=checkbox]').change(function(){
   ...
   minDate:'<?php echo $class_start_date; ?>',
    ...
    autoApply: true
      ...
      classDate = start.format('MM/DD/YYYY')+'-'+end.format('MM/DD/YYYY');
      countFilterData();
...
category = (function() {
    // Get all selected checkboxes for category
    const selectedCategories = $.map($('input[name="category"]:checked'), function(c) { return c.value; });
    
    // If both are selected, return an empty string; otherwise, return the selected value
    return selectedCategories.length === 2 ? "" : selectedCategories[0] || "";
})();
